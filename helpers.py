...
# Set path for inspection data and rejected part list
INSPECTION_PATH = os.path.join(DATA_ROOT, 'inspection_data')
REJECTION_PATH = os.path.join(DATA_ROOT, 'rejected.csv')
...
class InspectionDatabase():
	...
	def bivarplot(self, df, x, y, title='Title', xlabel=None, ylabel=None, 
		...
		sns.displot(data=df, x=x, y=y, cbar=True)
		plt.title(title)
		plt.xticks(rotation=45)
		plt.xlabel(xlabel) if xlabel else None
